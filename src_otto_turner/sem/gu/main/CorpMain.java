...
public class CorpMain {
    ...
    private final int DIR_BENEFIT = 6;
    private final int PROMOTE = 7;
    ...
    private final int NET_ASC = 3;
    private final int NET_DESC = 4;

    public CorpMain() {
        io = new IOManager(System.in, System.out);
        Director.setBenefit(DEFAULT_DIRECTOR_BENEFIT);
        employeeController = new EmployeeController(io);
    }
    ...
    public Employee retrieveEmployee(int id) {
        ...
        int i = 0;
        while (i < employeeController.getSize() && returnEmp == null) {
            Employee tempEmp = employeeController.getEmployee(i);
            if (tempEmp.getId() == id) {
    ...
    public boolean checkEmployeeExists(int id) {
        ...
        int i = 0;
        while (i < employeeController.getSize() && exists == false) {
            if (employeeController.getEmployee(i).getId() == id) {
                exists = true;
            }
            ++i;
        }
            ...
            switch (option) {
                case REGISTER:
                    ...
                    if (employee != null) {
                        ...
                        io.printf(">>> EMPLOYEE REGISTERED. THERE IS NOW %d EMPLOYEE(S) REGISTERED%n", employeeController.getSize());
                    }
                    break;
                case REMOVE:
                ...
                case RETRIEVE:
                    ...
                    break;
                case PRINT:
                    io.printf(">>> OPTION %d SELECTED: PRINT ALL EMPLOYEES", PRINT);
                    io.println(">>>");
                    printSorted();
                    break;
                case UPDATE_INFO:
    ...
    public void printMenu() {
        ...
        io.printf(">>> %d. PRINT ALL EMPLOYEES SORTED%n", PRINT);
    ...
    public void removeEmployee() {
        ...
        for (int i = 0; i < employeeController.getSize(); i++) {
            if (employeeController.getEmployee(i).equals(id))
                ;
    ...
    public void promoteToManager(Employee em, DegreeType degree) {
        ...
        employeeController.removeEmployee(em);
        employeeController.addEmployee(new Manager(name, basicGrossSalary, id, degree));
    }
    ...
    public void promoteToDirector(Employee em, DegreeType deg, Department dep) {
        ...
        employeeController.removeEmployee(em);
        employeeController.addEmployee(new Director(name, basicGrossSalary, id, deg, dep));
    }
    ...
    public void promoteToIntern(Employee em, GPA gpa) {
        ...
        employeeController.removeEmployee(em);
        employeeController.addEmployee(new Intern(name, basicGrossSalary, id, gpa));
    }
    ...
    public void promoteToRegularEmployee(Employee em) {
        ...
        employeeController.removeEmployee(em);
        employeeController.addEmployee(new Employee(name, basicGrossSalary, id));
    }

    public void printSorted() {
        io.printf(">>> HOW SHOULD THE EMPLOYEES BE SORTED?");
        int order = io.inputPositiveInteger(String.format(">>> ASCENDING NAME [%d], DESCENDING NAME [%d], " +
                        "ASCENDING NET SALARY [%d], OR DESCENDING NET SALARY [%d]?",
                        NAME_ASC, NAME_DESC, NET_ASC, NET_DESC));

        switch(order) {
            case 1:
                employeeController.sortBy(EmployeeController.COMPARE_NAME, EmployeeController.ASCENDING);
                break;
            case 2:
                employeeController.sortBy(EmployeeController.COMPARE_NAME, EmployeeController.DESCENDING);
                break;
            case 3:
                employeeController.sortBy(EmployeeController.COMPARE_NET, EmployeeController.ASCENDING);
                break;
            case 4:
                employeeController.sortBy(EmployeeController.COMPARE_NET, EmployeeController.DESCENDING);
                break;
            default:
                io.println(">>> NO valid selection. Using default sorting.");
                employeeController.sortBy(EmployeeController.COMPARE_NAME, EmployeeController.ASCENDING);
                break;
        }

        employeeController.printAll();
    }
